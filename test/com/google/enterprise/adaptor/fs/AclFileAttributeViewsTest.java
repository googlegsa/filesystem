...
public class AclFileAttributeViewsTest {
  ...
  private AclFileAttributeView inheritedAclView() { 
    return new AclView(group("EVERYONE").type(ALLOW).perms(READ_DATA)
  ...
  public void testGetDirectAclView() throws Exception {
    assertEquals(directAclView().getAcl(),
                 aclViews.getDirectAclView().getAcl());
  }

  @Test
  public void testInheritedAclView() throws Exception {
    assertEquals(inheritedAclView().getAcl(),
  ...
  public void getCombinedAclViewNoInheritedAcl() throws Exception {
    AclFileAttributeViews aclViews =
        new AclFileAttributeViews(directAclView(), new AclView());
    assertEquals(directAclView().getAcl(),
                 aclViews.getCombinedAclView().getAcl());
  }

  @Test
  public void getCombinedAclViewNoDirectAcl() throws Exception {
    AclFileAttributeViews aclViews =
      new AclFileAttributeViews(new AclView(), inheritedAclView());
    assertEquals(inheritedAclView().getAcl(),
                 aclViews.getCombinedAclView().getAcl());
  }
}
